...
//	}
//
//	@DeleteMapping("/delete_duty/{ac_id}")
//	public ResponseEntity<String> deletedutyty(@PathVariable int dh_id) {
...
public class Duty_HistoryServiceController {
	...
	@GetMapping("/duties/{dh_id}")
	public ResponseEntity<Duty_History> getduty(@PathVariable int dh_id) {
		Duty_History p = dservice.getById(dh_id);
		if (p != null)
			return ResponseEntity.ok(p);
		else
	...
	@DeleteMapping("/delete_duty/{dh_id}")
